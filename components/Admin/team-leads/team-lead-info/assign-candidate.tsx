...
import useAssignCandidateToTeamLead from "@/features/team-lead/api/assign-candidate-to-team-lead";

const AssignCandidate = ({ candidate }: { candidate: CandidateSchema }) => {
  console.log(candidate);
  const router = useRouter();
  const teamLeadId = router.query.teamLeadId as string;
  const { mutate, isLoading, isError } = useAssignCandidateToTeamLead();
  const handleAssign = (candidateId: string) => {
    mutate({ candidateId, teamLeadId });
  };
  ...
  return (
    <Card key={crypto.randomUUID()} className="flex gap-[18px] p-5">
      <div className="">
        <CandidateCardAvatar
