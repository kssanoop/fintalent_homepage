...
import ShortlistJobApplication from "@/features/jobs/shortlist-job-application";
...
const InterviewScheduledCandidatesTab = () => {
  ...
  const interviewScheduledCandidates = data.pages.flatMap((pg) => pg.data);
  ...
  return (
    <div className="scroll-container h-[calc(100vh-118px)] space-y-2 overflow-auto">
      {interviewScheduledCandidates?.length === 0 ? (
        ...
        interviewScheduledCandidates?.map(
          (candidate: JobApplicationSchema, i) => (
            <Card
              key={candidate._id}
              ref={interviewScheduledCandidates.length === i + 1 ? ref : null}
...
export default InterviewScheduledCandidatesTab;
