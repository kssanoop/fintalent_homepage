...
import { useChangeTagState } from "@/features/tags/admin/api/change-state-of-tag";
...
const ClickedTagId = () => {
  ...
  const [isOn, setIsOn] = useState(tag?.docStatus === "active");
  const [filters, setFilters] = useState<CandidateFilters | {}>({});
  const prevIsOnRef = useRef(isOn);
  const queryClient = useQueryClient();

  console.log("docs status:", isOn, tag?.docStatus);

  const forSubmissionSuccess = () => {
    queryClient.invalidateQueries({ queryKey: ["tags-admin"] });
    queryClient.invalidateQueries({ queryKey: ["get-tag-by-id"] });
    toast.success("Tag Status changed Successfully");
  };
  const forSubmissionError = () => {
    toast.error("Error occured while changing tag status");
  };
  const { mutate } = useChangeTagState(
  ...
  useEffect(() => {
    if (prevIsOnRef.current !== isOn) {
      ...
      if (isOn) {
        mutate({ tagId, state: "active" });
      } else {
        mutate({ tagId, state: "inactive" });
      }
    }
  }, [isOn, mutate, tagId]);
            ...
            <div className="flex w-full justify-between gap-6 md:gap-[129px]">
              ...
              <div className="mr-6 flex items-center gap-1">
                <p className="text-sm font-normal leading-5 text-[#171717]">
                  Tag status : {isOn ? "inactivate" : "activate"}
                </p>
                <ToggleSwitch setIsOn={setIsOn} isOn={isOn} />
              </div>
            </div>
            {/* <DynamicHeightContainer> */}
